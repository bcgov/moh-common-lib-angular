...
/**
 * Builds a fixture for a test host component, replacing its template with the
 * one given. Auto change detection is on, so tests pair tick() with
 * tickAndDetectChanges() rather than calling detectChanges() themselves.
 *
 * The components under test are standalone, so the host goes in imports. An
 * earlier version of this helper used declarations, which standalone rejects,
 * and every spec that needed it kept a private copy instead.
 */
...
export function getInputDebugElement(
  de: DebugElement,
  name: string
): DebugElement {
...
export function getSelectDebugElement(
  de: DebugElement,
  name: string
): DebugElement {
...
export function setInput(
  ...
  const input = getInputDebugElement(
    de,
    labelName ? labelName : de.componentInstance.labelforId
  );
...
export function setSelect(
  ...
  const select = getSelectDebugElement(
    de,
    labelName ? labelName : de.componentInstance.labelforId
  );
