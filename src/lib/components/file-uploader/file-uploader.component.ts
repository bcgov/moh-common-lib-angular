...
/**
 * Drag-and-drop or browse upload for images and PDFs. PDFs are rasterised one
 * image per page. Oversized images are scaled down until they fit.
 *
 * PDF support needs pdfjs-dist, which is an optional peer dependency.
 *
 * @example
 * <common-file-uploader id="docs" [(images)]="images" [required]="true">
...
export class FileUploaderComponent extends Base {
  ...
  @Output() imagesChange: EventEmitter<Array<CommonImage>> = new EventEmitter<
    Array<CommonImage>
  >();
  @Output() errorDocument: EventEmitter<CommonImage> =
    ...
    switch (file.type) {
      case 'application/pdf':
        try {
          ...
          this.handleError(error as CommonImageError);
        }
        break;
  ...
  private processPDFFile(file: File): Promise<Array<CommonImage>> {
    ...
    return new Promise((resolve, reject) => {
      reader.onload = () => {
        this.pdfService.getDocument(reader.result as ArrayBuffer).then(
          async (pdfDoc: any) => {
            ...
            for (
              ...
              try {
                const image = await this.getPageImage(pdfDoc, pageNumber);
  ...
  private getPageImage(pdfDoc: any, pageNumber: number): Promise<CommonImage> {
  ...
  private async scaleImage(image: CommonImage): Promise<CommonImage> {
    return new Promise<CommonImage>((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const targetWidth: number = Math.floor(
          img.width * IMAGE_REDUCTION_SCALE_FACTOR
        );
        const targetHeight: number = Math.floor(
          img.height * IMAGE_REDUCTION_SCALE_FACTOR
        );
  ...
  private createImage(imageDataURL: any): Promise<CommonImage> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      images.forEach((image) => {
        const existingIndex = this.images.findIndex(
          (existingImage) => existingImage.fileContent === image.fileContent
        );
      ...
      this.images = [...this.images, ...imagesToAdd];
