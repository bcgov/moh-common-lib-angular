  ...
  implements OnInit, OnChanges, ControlValueAccessor
{
  ...
  removeLine(line: 2 | 3) {
    ...
    const lookup = `showLine${line}`;
    (this as any)[lookup] = false;

    // TODO - Need to clear the data in the appropriate field, just null/undefined it out.
    const addrLookup = `addressLine${line}`;
    (this.address as any)[addrLookup] = undefined;
  }
  ...
  private setDefaultProvinceAsOption(country: string): string {
    const provObj = !this.provinceList
      ...
      : this.provinceList.find(
          (val) =>
            (val.provinceCode === this.defaultProvince ||
  ...
  private findCountryCode(country: string): string | null {
    const countryObj = !this.countryList
      ? null
      : this.countryList.find(
          (val) => val.countryCode === country || val.description === country
        );
  ...
  selectSuggestedAddress(address: any) {
  ...
  private setLabels() {
    if (this.labels) {
      (Object.keys(this.labels) as (keyof AddrLabelList)[]).forEach((x) => {
        this.addrLabels[x] = this.labels[x];
      });
    }
  }
  ...
  private setMaxlengths() {
    if (this.maxlengths) {
      (Object.keys(this.fieldMaxLengths) as (keyof Maxlengths)[]).forEach(
        (x) => {
          this.maxlengths[x] = this.fieldMaxLengths[x];
        }
      );
    }
  }
  ...
  private setReadOnlyFields() {
    if (typeof this.disabled === 'boolean') {
      Object.keys(this.readOnlyFields).map(
        (x) => (this.readOnlyFields[x] = this.disabled)
      );
    } else {
      Object.keys(this.disabled).map(
        (x) =>
          (this.readOnlyFields[x] =
  ...
  private truncateAddressLines(address: Address): Address {
    ...
    // };
    for (let i = 1; i <= 3; i++) {
      const addressLineName = 'addressLine' + i;
      if ((address as any)[addressLineName]) {
        lines.push((address as any)[addressLineName]);
      }
    }
