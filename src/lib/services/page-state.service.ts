...
export class PageStateService {
  ...
  findIndex( url: string  | null = null ): number {
    let idx = 0;
    if ( this.pageList ) {
  ...
  setPageIncomplete( url: string | null = null ) {
    const obj = this._find(url ? url : this.router.url);
    if ( obj ) {
  ...
  setPageComplete( path: string  | null = null ) {
  ...
  canNavigateToPage( path: string  | null  = null ): boolean {
    let complete = false;
    const obj =  this._find(path ? path : this.router.url);
    if ( obj ) {
